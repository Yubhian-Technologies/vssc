...
const PAGE_NAME = "InsideOut";
const EVENTS_COLLECTION = "insideout_events";
...
const InsideOutPage: React.FC = () => {
    ...
    const unsub = onAuthStateChanged(auth, async (user) => {
      if (user && user.emailVerified) {
        ...
        if (snap.exists()) {
          ...
          setRegForm((prev) => ({ ...prev, name: data.name || user.displayName || "" }));
        }
      } else {
        setCurrentUser(null);
        setUserRole("student");
  ...
  // NEW: Validate edit datetime >= original AND >= now (IST)
        ...
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <DialogFooter>
              ...
              <Button variant="outline" onClick={() => setShowRegModal(false)} className="bg-red-600 text-white">
                Cancel
              </Button>
            </DialogFooter>
          </form>
              ...
              {participants.map((p) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    {p.message && <p className="text-xs text-gray-500 mt-1">"{p.message}"</p>}
                  </div>
                </div>
              ))}
            </div>
