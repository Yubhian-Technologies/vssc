...
export default function StudyWorkshopPage() {
    ...
    try {
      if (selectedSession.isGroup) {
        await runTransaction(db, async (transaction) => {
          ...
          if (!sessionData.slots || sessionData.slots <= 0)
            throw new Error("No slots left.");
          if (sessionData.participants?.includes(user.uid))
            throw new Error("You already joined this group session.");
  ...
  const handleAddSession = async () => {
    if (
      ...
      toastError(
        "Please fill in all required fields (title, description, tutor name, skills, date, start time, total duration, slots, expiry date, and expiry time).",
      );
      return;
    }
              ...
              <form className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-gray-500">
                    Study workshops are group sessions only
                  </p>
                </div>
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        className="w-full p-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent transition-all duration-200 bg-white"
                        value={
                          newSession.totalDuration === 0
                            ? ""
                            : newSession.totalDuration
                        } // Display empty string if 0
                        onChange={(e) =>
                          setNewSession({
                            ...newSession,
                            totalDuration:
